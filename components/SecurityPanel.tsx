...
type Team = { id: string; name: string; hasManagerPassword: boolean };
...
export function SecurityPanel({ teams: initialTeams }: { teams: Team[] }) {
  ...
  async function handleClearTeamPassword(teamId: string) {
    ...
    setSavingTeamId(null);
    if (res.ok) {
      setTeams((prev) => prev.map((t) => (t.id === teamId ? { ...t, hasManagerPassword: false } : t)));
    }
  }
  ...
  return (
    <div>
      ...
      <section className="mb-8 rounded-2xl border border-line bg-surface p-6">
        <h2 className="mb-1 font-bold">Admin password</h2>
      ...
      <section className="rounded-2xl border border-line bg-surface p-6">
        <h2 className="mb-1 font-bold">Team manager logins</h2>
            ...
            <li key={t.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-40 rounded-lg border border-line bg-background px-3 py-1.5 text-sm outline-none focus:border-pitch"
                />
                <button
                  onClick={() => handleSetTeamPassword(t.id)}
                  disabled={savingTeamId === t.id}
                  className="rounded-full bg-pitch px-4 py-1.5 text-xs font-bold text-white disabled:opacity-60"
                >
                ...
                {t.hasManagerPassword && (
                  <button
                    ...
                    disabled={savingTeamId === t.id}
                    className="rounded-full border border-line px-4 py-1.5 text-xs font-medium text-live disabled:opacity-40"
                  >
                    Clear
                  </button>
                )}
